...
def create_model(batch_size, seq_len, d_k, d_v, n_heads, ff_dim):
    ...
    cross_attn_layer1 = TransformerEncoder(d_k, d_v, n_heads, ff_dim)
    cross_attn_layer2 = TransformerEncoder(d_k, d_v, n_heads, ff_dim)
    cross_attn_layer3 = TransformerEncoder(d_k, d_v, n_heads, ff_dim)
    ...
    ob_x = ob_attn_layer1((x_conv, x_conv, x_conv))
    ob_x = ob_attn_layer2((ob_x, ob_x, ob_x))
    ob_x = ob_attn_layer3((ob_x, ob_x, ob_x))


    tr_x = tr_attn_layer1((tr_in_seq, tr_in_seq, tr_in_seq))
    tr_x = tr_attn_layer2((tr_x, tr_x, tr_x))
    tr_x = tr_attn_layer3((tr_x, tr_x, tr_x))

    cross = cross_attn_layer1((ob_x, tr_x, tr_x))
    cross = cross_attn_layer2((cross, cross, cross))
    cross = cross_attn_layer3((cross, cross, cross))
